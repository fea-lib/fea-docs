...
- [x] **Z3 — FrameworkAdapter contract.** **Single adapter per framework** (the one render backend seam), exposing: **(a)** `render` → static HTML from the page's component set, **(b)** `hydration` (hybrid only) → page chunk + shared vendor chunk ref + mount markers. Framework key from the **component declaration context** (per-file import graph; `renderMode`/framework on the usage — v1 always `'react'`, seam exists for Vue/Solid). Adapter owns mount markers + vendor-vs-page split — future frameworks supply their own without consumer changes.
- [x] **Z4 — Prototype verdict (spike run evidence).** **Confirmed:** static-only pages ship zero JS; hybrid pages ship one page chunk + a shared vendor chunk; React's server render executes `useState`/`useMemo`/`useCallback` at build; the adapter/render seam holds. **Correction discovered 1:** `useEffect`/`useLayoutEffect`/`useInsertionEffect`/`useImperativeHandle` are hardcoded `noop` in React's server runtime — effects do **not** run at build, so their mutations never reach static HTML. A hybrid component yields *pre-effect* static HTML; its effect behavior exists only in the hydrated client pass. **Correction discovered 2 (from the hybrid example):** a renderer that emits **only the body fragment is incomplete** — the produced page must be a full HTML doc *and* carry the `<script>` references to the chunks that must load, or hydration never executes. The spike renderer now assembles the full page shell (`<title>` from first H1, body) + `<script type="module">` for `vendor.js` and the page chunk; hybrid roots carry `data-hydrate` markers (the `renderMode` prop passes through MDX) as the mount contract. Implications: (a) static HTML for hybrid components is correct to the pre-effect state; (b) the "ships full transitive graph" guarantee + the mounted page chunk is what makes hydration possible; (c) **05-opener**: hydration runs the page chunk, wired to the shared `vendor.js` via esbuild `external` rather than re-bundling react per page. **Byte reality (full shell):** static page 349/257; hybrid page 549/326; page chunk 61314; vendor 60186 — static = 0 JS; page chunk/vendor currently both include react (esbuild `bundle`), 05 dedups via external. **Feed-back to 02 D4:** component-rendered headings are exercised on *pre-effect* state — effect-generated heading text cannot exist server-side; add to the D4 revisit scope.

- [ ] Static `renderMode="static"` component renders to correct static HTML (no client JS needed)
- [ ] `renderMode="hybrid"` component renders correct HTML at build for **pre-effect state** (effects are server `noop` — verified) and its full transitive graph ships in the page chunk
...
- [ ] Wrapper contract verified: one adapter per framework, invoked with enough signal to swap to Vue/Solid later without consumer changes
