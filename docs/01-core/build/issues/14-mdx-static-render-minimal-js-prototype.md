...
- [x] **Z4 — Prototype verdict (spike run evidence).** **Confirmed:** static-only pages ship zero JS; hybrid pages ship one page chunk + share one vendor chunk; React's server render executes `useState`/`useMemo`/`useCallback` at build; the adapter/render seam holds. **Correction discovered:** `useEffect`/`useLayoutEffect`/`useInsertionEffect`/`useImperativeHandle` are hardcoded `noop` in React's server runtime ([react-dom-server…node.development.js: `useEffect: noop`]) — effects do **not** run at build, so their mutations never reach static HTML. A hybrid component yields *pre-effect* static HTML; its effect behavior exists only in the hydrated client pass. Implications: (a) static HTML for hybrid components is correct to the pre-effect state; (b) the "ships full transitive graph" guarantee is precisely what makes hydration possible; (c) **05-opener**: hydration runs the page chunk (the component's full graph) — the "did-run vs must-ship" distinction closes here. **Byte reality:** static page 153 B raw / 141 gz; hybrid page 202 / 149; page chunk 170 / 130; vendor 120 / 110 — static = 0 JS shipped; vendor paid once, shared. **Feed-back to 02 D4:** component-rendered headings are exercised on *pre-effect* state — effect-generated heading text cannot exist server-side; add to the D4 revisit scope.
