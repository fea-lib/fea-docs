...
- [x] **D4 — Heading anchors (part).** `github-slugger` wrapped in one slug module (`src/render/slugs.ts`); every consumer (heading ids here, 07 fragment rewriting, 11 index sections) goes through it — pass-through today, single extension point. Bare `id=""` edge cases **accepted as-is for now**. Additionally, `rehype-raw` makes literal raw-HTML headings traversable, so markdown *and* raw-HTML headings are id'd and feed first-H1. **Component-rendered headings** and the full slug-coverage question (which headings are covered/missed) are **revisited once the build-time render solution is established — dependent on `issues/14-mdx-static-render-minimal-js-prototype.md`**. **Prototype verdict input (ticket 14):** React static render executes compilers but **effects are server `noop`s** — a component that produces heading text via an effect yields *pre-effect* HTML at build. Re-run of the revisit must therefore cover: component-authored headings are extractable from the *pre-effect* static tree only; effect-generated heading text **cannot exist server-side** — defer that coverage question to 05's hydration contract. Markdown + literal raw-HTML headings remain claimed here.
